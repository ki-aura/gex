...
{
	return (unsigned char) ((HELPER_nib_to_hexval(hi) << 4) |
				HELPER_nib_to_hexval(lo));
}
...
{
	if ((hex.v_start + byte_offset) >= app.fsize)
		return false;	// >= as offset is zero based so must be 1 less than size
...
unsigned long cursor_full_file_offset(void)
{
	...
	unsigned long idx =
	    hex.v_start + row_digit_to_offset(hex.cur_row, hex.cur_digit);
...
const char *get_filename(const char *path)
{
	...
	if (last)
		return last + 1;	// character after last '/'
...
void *xcalloc(size_t count, size_t size)
{
	...
	if (ptr == NULL && count > 0 && size > 0) {
		fprintf(stderr,
			"Fatal: Out of memory (calloc %zu count %zu bytes).\n",
			count, size);
		exit(EXIT_FAILURE);
	}
	return ptr;
