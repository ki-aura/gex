...
void create_windows(void)
{
	...
	if (app.too_small) {
		...
		mvprintw(app.rows / 2, (app.cols - 45) / 2,
			 "Screen is too small. Please resize to continue.");
		...
		status.border =
		    newwin(status.height + 2, status.width + 2, 0, 0);
		hex.border =
		    newwin(hex.height + 2, hex.width + 2, status.height + 2, 0);
		ascii.border =
		    newwin(ascii.height + 2, ascii.width + 2, status.height + 2,
			   hex.width + 2);
		...
		ascii.win =
		    newwin(ascii.height, ascii.width, status.height + 3,
			   (hex.width + 3));
	}
...
{
	mvwprintw(status.win, 0, 0,
		  "GEX %s [%s] Size:%lu Offset:%lu             ", GEX_VERSION,
		  get_filename(app.fname), app.fsize,
		  cursor_full_file_offset());
	mvwprintw(status.win, 1, 0,
		  "Grid offset %lu-%lu Screen:%dx%d Grid:%dx%d=%d           ",
		  hex.v_start, hex.v_start + hex.grid - 1, app.rows, app.cols,
		  ascii.width, hex.height, hex.grid);
	mvwprintw(status.win, 2, 0,
		  "cr%02d cc%02d cd%02d Hwin?%d hinib?%d lk%d lek%d chg%0d rbe?%d        ",
		  hex.cur_row, hex.cur_col, hex.cur_digit, app.in_hex,
		  hex.is_hinib, app.lastkey, app.lasteditkey, RB_SIZE(),
		  RB_EMPTY(&edits));
	box(status.border, 0, 0);
...
void refresh_grids(void)
{
	...
	while (file_offset_to_rc(offset, &r, &hc, &ac)) {	//while v_start + offset is < end of file
		...
		// update the grids
		if (chg)
			wattron(hex.win, COLOR_PAIR(1) | A_BOLD);
		mvwprintw(hex.win, r, hc, "%c", hinib);
		mvwprintw(hex.win, r, hc + 1, "%c", lonib);
		if (chg)
			wattroff(hex.win, COLOR_PAIR(1) | A_BOLD);

		if (chg)
			wattron(ascii.win, COLOR_PAIR(1) | A_BOLD);
		mvwprintw(ascii.win, r, ac, "%c", a_char);
		if (chg)
			wattroff(ascii.win, COLOR_PAIR(1) | A_BOLD);
		offset++;
	}
...
unsigned long popup_question(const char *qline1, const char *qline2,
			     popup_types pt)
{
	...
	qlen =
	    (int) ((strlen(qline1) >
		    strlen(qline2)) ? strlen(qline1) : strlen(qline2));
