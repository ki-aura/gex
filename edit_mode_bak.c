...
DP_ON=true;		
		...
		idx = hex.cur_row * hex.digits + hex.cur_digit;
                if (!app.in_hex) { // we're in ascii pane
                    if (isprint(k)) {
                        hex.gc_copy[idx] = (unsigned char)k;
                    ...
                    if ((k >= '0' && k <= '9') || (k >= 'A' && k <= 'F') || (k >= 'a' && k <= 'f')) {
                        ...
                        if (hex.is_lnib) {
sprintf(tmp, "hi nib idx %i k %c gc_cop %c ", idx, k, hex.gc_copy[idx]); DP(tmp); 
                            hex.gc_copy[idx] = (hex.gc_copy[idx] & 0x0F) | (nibble << 4);
                            hex.is_lnib = false;
                            hex.changes_made = true;
                            e_handle_keys(KEY_RIGHT);
                            e_refresh_hex();
                        } else {
sprintf(tmp, "low nib idx %i k %c gc_cop %c ", idx, k, hex.gc_copy[idx]); DP(tmp); 
                            hex.gc_copy[idx] = (hex.gc_copy[idx] & 0xF0) | nibble;
                            hex.is_lnib = true;
                            hex.changes_made = true;
                            e_handle_keys(KEY_RIGHT);
                            e_refresh_hex();
